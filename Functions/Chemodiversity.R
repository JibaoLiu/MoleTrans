...
dataPrecess_Formularity <- function(data,Deu_DT=FALSE){
  ...
  if (!str_detect(str_flatten(colnames(data)),"AImod")){
    if(!any(colnames(data) %in% "Br")){
      ...
      if(any(data$DBEAI<=0)){
        data[data$DBEAI<=0,]$AImod <- 0
      }
      if(any(data$CAI<=0)){
        data[data$CAI<=0,]$AImod <- 0
      }
...
KVD <- function(userdata){
  ...
  Others <- userdata[!userdata$`Measured m/z`%in%KVDdata$`Measured m/z`,]
  if(!nrow(Others)==0){
...
FormuTrans <- function(formula_ls){
  ...
  if (ncol(formula_ls)>1){
    ...
    Formula_df <- tibble(Formula = NA, C = NA, H=NA, D=NA, N=NA, O=NA, P=NA, S=NA, Cl=NA, Br = NA, I=NA, F=NA)
    for (i in 1:nrow(formula_ls)) {
      ...
      if (!(str_detect(formula,"n")|str_detect(formula,"o")|str_detect(formula,"a")|str_detect(formula,"\\(")|str_detect(formula,"\\.")|is.na(formula))){
        formula_splitforC <- str_split(formula,"")[[1]]

        if(str_detect(formula,"C")){
          if(any(is.na(formula_splitforC[which(formula_splitforC=="C")+1]))){
            if (str_detect(formula,"C")){
            ...
            if (!all(formula_splitforC[which(formula_splitforC=="C")+1]=='l')){
              C_index <- which(formula_split[[1]]%in%"C")[1]
              if (is.na(split_num1[C_index+1])){
                C_num <- 1
              }
        ...
        if (str_detect(formula,"D")){
          D_index <- which(formula_split[[1]]%in%"D")
          if (is.na(split_num1[D_index+1])){
            D_num <- 1
          }
          else if (!is.na(split_num1[D_index+2])){
            D_num <- split_num[D_index+1]*10+split_num[D_index+2]
          } else {D_num <- split_num[D_index+1]}
        } else {D_num <- 0}
        ...
        Formula_temp <- tibble(Reaction = formula_ls$Reaction[i], Formula = formula_ls$Formula[i], C = C_num, H=H_num, D=D_num, N=N_num, O=O_num, P=P_num, S=S_num,Cl=Cl_num, Br=Br_num, I=I_num, F=F_num)
      } else {
        Formula_temp <- tibble(Reaction = formula_ls$Reaction[i], Formula = formula_ls$Formula[i], C = NA, H=NA,D=NA, N=NA, O=NA, P=NA, S=NA, Cl=NA, Br=NA, I=NA, F=NA)
      }
    ...
    for (i in 1:nrow(formula_ls)) {
      ...
      if (!(str_detect(formula,"n")|str_detect(formula,"o")|str_detect(formula,"a")|str_detect(formula,"\\(")|str_detect(formula,"\\.")|is.na(formula))){
        ...
        Formula_temp <- tibble(Formula = formula, C = C_num, H=H_num,  D=D_num, N=N_num, O=O_num, P=P_num, S=S_num,Cl=Cl_num, Br=Br_num, I=I_num, F=F_num)
      } else {
        Formula_temp <- tibble(Reaction = formula_ls$Reaction[i], Formula = formula_ls$Formula[i], C = NA, H=NA, D=NA,N=NA, O=NA, P=NA, S=NA, Cl=NA, Br=NA, I=NA, F=NA)
      }
      Formula_df <- rbind(Formula_df,Formula_temp)
...
VKDGrah5 <- function(data,pointsize=TRUE){
  if (pointsize){
    P1Element1_size_legend <- ggscatter(data,'O/C','H/C',shape = 21,color = 'black',fill = 'Group',size = 'ReInt',alpha = 0.8,stroke=0.2)+
      scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+scale_size(range = c(0.8,3))+xlim(0,1.2)+ylim(0,2.5)+
      theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+
      theme(legend.position = c(0.7, 0.95),legend.direction = 'horizontal')+guides(fill='none') 
    P1Element1_size_legend <- P1Element1_size_legend+geom_hline(aes(yintercept=1.5),linetype="longdash",color="darkgrey",linewidth=0.1)+
    ...
    P1Element1_fill_legend <- ggscatter(data,'O/C','H/C',shape = 21,color = 'black',fill = 'Group',alpha = 0.8,stroke=0.2)+
      scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+scale_size(range = c(0.8,3))+
      theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+
      theme(legend.position = 'top',legend.key.height=unit(0.2,"line"),legend.margin = ggplot2::margin(2,0,0,0),legend.box.margin=ggplot2::margin(2,-10,0,-10),plot.margin = unit(c(0.2,1,0.6,0.6),'cm'))+
      guides(fill=guide_legend(override.aes = list(size=2)))
    ...
    P1Element1 <- ggscatter(data,'O/C','H/C',fill = 'Group',shape = 21,color = 'black',size = 0.8,alpha = 0.8,stroke=0.2)+
      scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+xlim(0,1.2)+ylim(0,2.5)+
      theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+
      theme(legend.position = 'top',legend.key.height=unit(0.2,"line"),legend.margin = ggplot2::margin(2,0,0,0),legend.box.margin=ggplot2::margin(2,-10,0,-10),plot.margin = unit(c(0.2,1,0.6,0.6),'cm'))+
      guides(fill=guide_legend(override.aes = list(size=2)))
...
VKDGrah6 <- function(data,pointsize=TRUE){
  if (pointsize){
    P1Element1_size_legend <- ggscatter(data,'O/C','H/C',shape = 21,color = 'black',fill = 'Group',size = 'ReInt',alpha = 0.8,stroke=0.2)+
      scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+scale_size(range = c(0.8,3))+xlim(0,1.2)+ylim(0,2.5)+
      theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+theme(legend.position = c(0.7, 0.95),legend.direction = 'horizontal')+guides(fill='none')
    P1Element1_size_legend <- P1Element1_size_legend+geom_hline(aes(yintercept=2),linetype="longdash",color="darkgrey",linewidth=0.1)+
      ...
      geom_abline(intercept = 1.05,slope = -0.38,linetype="longdash",color="darkgrey",linewidth=0.1)+
      geom_abline(intercept = 0.8,slope = -0.38,linetype="longdash",color="darkgrey",linewidth=0.1)
    P1Element1_fill_legend <- ggscatter(data,'O/C','H/C',shape = 21,color = 'black',fill = 'Group',alpha = 0.6,stroke=0.2)+
      scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+scale_size(range = c(0.8,3))+
      theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+
      theme(legend.position = 'top',legend.key.height=unit(0.2,"line"),legend.margin = ggplot2::margin(2,0,0,0),legend.box.margin=ggplot2::margin(2,-10,0,-10),plot.margin = unit(c(0.2,1,0.6,0.6),'cm'))+
      guides(fill=guide_legend(override.aes = list(size=2)))
  ...
  if(biochemcal_type=="NewChemgroup"){
    if (pointsize){
      P1Element1_size_legend <- ggscatter(datatmp0,'O/C','H/C',shape = 21,color = 'black',fill = 'Sample',size = 'ReInt',alpha = 0.8,stroke=0.2)+gghighlight(Sample %in% highlight_filter)+
        scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+scale_size(range = c(0.5,1.5))+xlim(0,1.2)+ylim(0,2.5)+
        theme_prism(base_size = 7,border = TRUE,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50)+
        theme(legend.position = 'top',legend.key.height=unit(0.2,"line"),legend.margin = ggplot2::margin(2,0,0,0),legend.box.margin=ggplot2::margin(2,-10,0,-10),plot.margin = unit(c(0.2,1,0.6,0.6),'cm'))+
        guides(fill=guide_legend(override.aes = list(size=2)))
      P1Element1Up <- P1Element1_size_legend+geom_hline(aes(yintercept=2),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_hline(aes(yintercept=1.5),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_abline(intercept = 1.05,slope = -0.38,linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_abline(intercept = 0.8,slope = -0.38,linetype="longdash",color="darkgrey",linewidth=0.1)
    ...
    if (pointsize){
      P1Element1_size_legend <- ggscatter(datatmp0,'O/C','H/C',shape = 21,color = 'black',fill = 'Sample',size = 'ReInt',alpha = 0.8,stroke=0.2)+gghighlight(Sample %in% highlight_filter)+
        ...
        theme(legend.position = 'top',legend.key.height=unit(0.2,"line"),legend.margin = ggplot2::margin(2,0,0,0),legend.box.margin=ggplot2::margin(2,-10,0,-10),plot.margin = unit(c(0.2,1,0.6,0.6),'cm'))+
        guides(fill=guide_legend(override.aes = list(size=2)))
      P1Element1Up <- P1Element1_size_legend+geom_hline(aes(yintercept=1.5),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_linerange(aes(xmin=0,xmax=0.3,y=2.0),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_linerange(aes(x=0,ymin=1.5,ymax=2.0),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_linerange(aes(x=0.3,ymin=1.5,ymax=2.0),linetype="longdash",color="darkgrey",linewidth=0.1)+
        geom_linerange(aes(x=0.3,ymin=1.5,ymax=2.2),linetype="longdash",color="darkgrey",linewidth=0.1)+
...
RelativeAbund <- function(FtMSDataList,SampleOrder=NA){
  ...
  for (i in 1:length(FtMSDataList)) {
    ...
    data_countElem <- data_countElem[!is.na(data_countElem$n),]
    ...
    data_countKVD <- data_countKVD[!is.na(data_countKVD$n),]
    ...
    data_countKVD2 <- data_countKVD2[!is.na(data_countKVD2$n),]
    ...
    data_sumIntElem <- data_sumIntElem[!is.na(data_sumIntElem$x),]
    ...
    data_sumIntKVD <- data_sumIntKVD[!is.na(data_sumIntKVD$x),]
    ...
    data_sumIntKVD2 <- data_sumIntKVD2[!is.na(data_sumIntKVD2$x),]
  ...
  p1<-ggpar(ggbarplot(res_countElem,'Sample','Relat', fill = 'Group',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label=FALSE, lab.size = 1,lab.pos = 'in')+
              xlab(NULL)+ylab("Count proportion (%)")+
              theme(axis.text.x = element_text(vjust=1,size = 5.5))+
              geom_label_repel(aes(fill = Group,label = Relat),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
  ...
  p2<-ggpar(ggbarplot(res_countElem,'Sample','n',fill = 'Group',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
              xlab(NULL)+ylab("Count")+
              theme(axis.text.x = element_text(vjust=1,size = 5.5))+
              geom_label_repel(aes(fill = Group,label = n),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
  ...
  p3 <- ggpar(ggbarplot(res_countKVD,'Sample','Relat', fill = 'groupKVD',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
                fill_palette('jco')+xlab(NULL)+ylab("Count proportion (%)")+
                theme(axis.text.x = element_text(vjust=1,size = 5.5))+
                geom_label_repel(aes(fill = groupKVD,label = Relat),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
                theme_prism(base_size = 9,border = T,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/70)+
                guides(fill=guide_legend(keywidth=0.4,keyheight=0.3,size=8/70)),legend = 'top',x.text.angle = 20)
  #ggsave("./output/res_countKVD.pdf",width=8.5,height=7,dpi=300,units = 'cm')
  p4 <- ggpar(ggbarplot(res_countKVD,'Sample','n', fill = 'groupKVD',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
                fill_palette('jco')+xlab(NULL)+ylab("Count")+
                theme(axis.text.x = element_text(vjust=1,size = 5.5))+
                geom_label_repel(aes(fill = groupKVD,label = n),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
                theme_prism(base_size = 9,border = T,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/70)+
                guides(fill=guide_legend(keywidth=0.4,keyheight=0.3,size=8/70)),legend = 'top',x.text.angle = 20)
  ...
  p5 <- ggpar(ggbarplot(res_countKVD2,'Sample','Relat', fill = 'NewChemgroup',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
                fill_palette('jco')+xlab(NULL)+ylab("Count proportion (%)")+
                theme(axis.text.x = element_text(vjust=1,size = 5.5))+
                geom_label_repel(aes(fill = NewChemgroup,label = Relat),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
                theme_prism(base_size = 9,border = T,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/70)+
                guides(fill=guide_legend(keywidth=0.4,keyheight=0.3,size=8/70)),legend = 'top',x.text.angle = 20)
  #ggsave("./output/res_countKVD2.pdf",width=8.5,height=7,dpi=300,units = 'cm')
  p6 <- ggpar(ggbarplot(res_countKVD2,'Sample','n', fill = 'NewChemgroup',size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
                fill_palette('jco')+xlab(NULL)+ylab("Count")+
                theme(axis.text.x = element_text(vjust=1,size = 5.5))+
                geom_label_repel(aes(fill = NewChemgroup,label = n),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
                theme_prism(base_size = 9,border = T,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/70)+
                guides(fill=guide_legend(keywidth=0.4,keyheight=0.3,size=8/70)),legend = 'top',x.text.angle = 20)
  ...
  p7 <- ggpar(ggbarplot(res_sumIntElem,'Sample','Relat', fill = 'Group', size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
  ...
  p8 <- ggpar(ggbarplot(res_sumIntKVD,'Sample','Relat', fill = 'groupKVD', size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
  ...
  p9 <- ggpar(ggbarplot(res_sumIntKVD2,'Sample','Relat', fill = 'NewChemgroup', size =8/70,alpha=0.6, width = 0.4, order = SampleOrder,label = FALSE,lab.size = 1,lab.pos = 'in')+
                fill_palette('npg')+xlab(NULL)+ylab("Relative abundance (%)")+
                theme(axis.text.x = element_text(vjust=1,size = 5.5))+
                geom_label_repel(aes(fill = NewChemgroup,label = Relat),show.legend = FALSE,fontface = "plain",size=2,alpha=0.8,box.padding = 0.2,segment.size = .5, point.padding = 0.2, hjust= -0.3,position = position_stack(vjust = 1),direction = 'y',max.overlaps = 6)+
                #geom_label_repel(aes(label = Relat),segment.size=0.1, label.r=0.02,fontface = "plain",label.size = 8/80,size=0.6, label.padding = 8/90,box.padding = 8/90)+
