...
f_calc_masses_dif_direct <- function(peaklist){
  diff_matrix <- outer(peaklist$`Measured m/z`,peaklist$`Measured m/z`,'-')
  #diff_matrix <- abs(diff_matrix)
  #diff_matrix[!upper.tri(diff_matrix,diag = TRUE)] <- 0
  diff_DF <-  as.data.frame(diff_matrix)
...
f_calc_masses_dif_betw_sam_directed <- function(DF1,DF2){
  ...
  pmd_result <- gather(diff_DF,Mass2,PMD,-ncol(diff_DF))
  pmd_result$PMD <- -pmd_result$PMD
  pmd_result <- setDT(pmd_result)
  pmd_result <- pmd_result[abs(PMD)>1e-3]
  pmd_result$Mass2 <- as.numeric(pmd_result$Mass2)
  return(pmd_result)
}
...
f_calc_masses_dif_betw_sam <- function(DF1,DF2){
  ...
  if(any(DF1$`Measured m/z` %in% DF2$`Measured m/z`)&FALSE){
    mz_inter <- unique(inner_join(DF1,DF2,by="Measured m/z")$`Measured m/z`)
    diff_matrix_inter <- outer(mz_inter,mz_inter,'-')
    diff_matrix_inter  <- abs(diff_matrix_inter)
    diff_matrix_inter [!upper.tri(diff_matrix_inter ,diag = TRUE)] <- 0
    diff_DF_inter <-  as.data.frame(diff_matrix_inter)
    colnames(diff_DF_inter)<- mz_inter
    diff_DF_inter$Mass1 <- mz_inter
    diff_DF_inter <- unique(diff_DF_inter)
    pmd_result1 <- gather(diff_DF_inter,Mass2,PMD,-ncol(diff_DF_inter))
    pmd_result1 <- setDT(pmd_result1)
    pmd_result1 <- pmd_result1[PMD>0]
    
    mz_DF1 <- filter(DF1,!`Measured m/z` %in% mz_inter)$`Measured m/z`
    if(is_empty(mz_DF1)){
      mz_DF1 <-DF1$`Measured m/z`
    }
    mz_DF2 <- filter(DF2,!`Measured m/z` %in% mz_inter)$`Measured m/z`
    if(is_empty(mz_DF2)){
      mz_DF2 <-DF2$`Measured m/z`
    }
    
    diff_matrix <- outer(mz_DF1,mz_DF2,'-')
    diff_matrix <- abs(diff_matrix)
    #diff_matrix[!upper.tri(diff_matrix,diag = TRUE)] <- 0
    diff_DF <-  as.data.frame(diff_matrix)
    colnames(diff_DF)<- mz_DF2
    diff_DF$Mass1 <- mz_DF1
    diff_DF <- unique(diff_DF)
    pmd_result2 <- gather(diff_DF,Mass2,PMD,-ncol(diff_DF))
    pmd_result2 <- setDT(pmd_result2)
    if (!nrow(pmd_result2)==0){
      pmd_result2 <- pmd_result2[PMD>1e-3]
      pmd_result <- rbind(pmd_result1,pmd_result2)
    }else{
      pmd_result <- pmd_result1
    }
...
FindPMD <- function(peaklist,PMDDF,mserror=2,directed=FALSE){
  if(directed==FALSE){
    pmdlist <- f_calc_masses_dif(peaklist)
  }else{
    pmdlist <- f_calc_masses_dif_direct(peaklist)
  }
  ...
  for (i in 1:length(d)) {
    if(directed==FALSE){
      tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-abs(d[i]))*1000000/abs(PMD))%>% mutate(DiffPMD1=abs(PMD-abs(d[i])))
    }else{
      tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-d[i])*1000000/abs(PMD))%>% mutate(DiffPMD1=abs(PMD-d[i]))
    }
    if(mserror<0.005){
    ...
    #tmp <- filter(tmp,DiffPMD<=mserror)
    if(!nrow(tmp)==0){
      PMD_findtmp <- tmp[,c("Mass2","Mass1","PMD")]
...
FindPMD_betw_samp <- function(DF1,DF2,PMDDF,multiDistance=FALSE,directed=FALSE){
  PMDinfo <- PMDDF
  if(any(PMDDF$Group %in% '18O')|any(PMDDF$Group %in% '18O-1')){
    PMDDF <- PMDDF
  }else{
    colnames(PMDinfo)[which(colnames(PMDinfo) == "Group")] <- "Formula"
    PMDinfo1 <-  FormuTrans(select(PMDinfo,c('Formula')))
    PMDinfo1$PMDmass_ref <- abs(PMDinfo1$C*12+PMDinfo1$H*1.007825+PMDinfo1$D*2.014102+PMDinfo1$O*15.994915+PMDinfo1$N*14.003074+PMDinfo1$S*31.972071+PMDinfo1$P*30.973762+PMDinfo1$Cl*34.968852682+PMDinfo1$Br* 78.9183376 +PMDinfo1$I* 126.9044719)
    PMDinfo1[which(PMDDF$PMD < 0),]$PMDmass_ref <- -PMDinfo1[which(PMDDF$PMD < 0),]$PMDmass_ref
    PMDDF <- PMDinfo1 %>% select(c('Formula','PMDmass_ref'))
    colnames(PMDDF) <- c('Group','PMD')
  }
  ...
  if(directed==FALSE){
    ...
    pmdlist <- f_calc_masses_dif_betw_sam_directed(DF1,DF2)
  }
  ...
  for (i in 1:length(d)) {
    if(multiDistance==FALSE){
      if(directed==FALSE){
        #tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-abs(d[i]))*1000000/abs(PMD))
        tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-abs(d[i])))
      }else{
        #tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-d[i])*1000000/abs(PMD))
        tmp <- pmdlist%>% mutate(DiffPMD=abs(PMD-d[i]))
      }
      tmp <- filter(tmp,DiffPMD<=0.000001)
    }else{
      tmp <- pmdlist%>% mutate(DiffPMD=abs(1-PMD/(d[i]*round(PMD/d[i])))*1000000)
      tmp <- filter(tmp,DiffPMD<=0.001)
    }
    
    if(!nrow(tmp)==0){
      PMD_findtmp <- tmp[,c("Mass1","Mass2","PMD")]
      #if(directed==FALSE){
        #PMD_findtmp$PMDRef <- abs(d[i])
      #}else{
      PMD_findtmp$PMDRef <- d[i]
      #}
...
MassToNetworkFile <- function(FtmsDF,PMD,mserror=2,directed=FALSE){
  ...
  PMD_result <- FindPMD(FtmsDF,PMD,mserror=mserror,directed=directed)
...
MassToNetworkFile_betw_sam <- function(FtmsDF1,FtmsDF2,PMD,multiDistance=FALSE,directed = FALSE){
  ...
  PMD_result <- FindPMD_betw_samp(FtmsDF1,FtmsDF2,PMD,multiDistance=multiDistance,directed = directed)
  ...
  #LinkNodeLs <- PMDFormulaConsensusFilter(LinkNodeLs[[1]],LinkNodeLs[[3]],PMD)
  return(LinkNodeLs)
}
...
PMDFormulaConsensusFilter <- function(netlink,netnode,PMD){
  ...
  new_df$D.p <- new_df$D.y-new_df$D.x
  ...
  new_df$Cl.p <- new_df$Cl.y-new_df$Cl.x
  new_df$Br.p <- new_df$Br.y-new_df$Br.x
  new_df$I.p <- new_df$I.y-new_df$I.x
  colnames(new_df)<-str_remove_all(colnames(new_df),'.p')
  new_df$TransGroup <- paste('C',new_df$C,'H',new_df$H,'D',new_df$D,'O',new_df$O,'N',new_df$N,'P',new_df$P,'S',new_df$S,'Cl',new_df$Cl,'Br',new_df$Br,'I',new_df$I,sep = '') %>% str_remove_all('C0')%>% str_remove_all('H0') %>%str_remove_all('D0') %>% str_remove_all('O0') %>% str_remove_all('N0')%>% str_remove_all('P0') %>% str_remove_all('S0')%>% str_remove_all('Cl0')%>% str_remove_all('Br0')%>% str_remove_all('I0')
  new_df$PMDmass <-abs(new_df$C*12+new_df$H*1.007825+new_df$D*2.014102+new_df$O*15.994915+new_df$N*14.003074+new_df$S*31.972071+new_df$P*30.973762+new_df$Cl*34.968852682+new_df$Br* 78.9183376 +new_df$I* 126.9044719)
  PMDinfo <-  PMD
  colnames(PMDinfo)[which(colnames(PMDinfo) == "Group")] <- "Formula"
  PMDinfo1 <-  FormuTrans(select(PMDinfo,c('Formula')))
  PMDinfo1$PMDmass_ref <- abs(PMDinfo1$C*12+PMDinfo1$H*1.007825+PMDinfo1$D*2.014102+PMDinfo1$O*15.994915+PMDinfo1$N*14.003074+PMDinfo1$S*31.972071+PMDinfo1$P*30.973762+PMDinfo1$Cl*34.968852682+PMDinfo1$Br* 78.9183376 +PMDinfo1$I* 126.9044719)
  netlink1 <- cbind(netlink,select(new_df,c('TransGroup','PMDmass')))
  netlink1 <- filter(netlink1, abs(netlink1$PMDmass) %in% abs(PMDinfo1$PMDmass_ref))
...
DiffNetwork <- function(FtmsDF1,FtmsDF2, PMD,masserror=0.00001,directed =FALSE){
  Net2 <- MassToNetworkFile_betw_sam(FtmsDF1,FtmsDF2,PMD,directed = directed)
  Net1 <- MassToNetworkFile_betw_sam(FtmsDF1,FtmsDF1,PMD,directed = FALSE)
...
DiffNetwork2 <- function(FtmsDF1,FtmsDF2, PMD,masserror=0.00001,directed = FALSE){
  FtmsDF1_2 <- rbind(FtmsDF1,FtmsDF2) 
  #Net3 <- MassToNetworkFile_betw_sam(FtmsDF2,FtmsDF2,PMD,directed = directed)
  Net2 <- MassToNetworkFile_betw_sam(FtmsDF1_2,FtmsDF2,PMD,directed = directed)
  Net1 <- MassToNetworkFile_betw_sam(FtmsDF1,FtmsDF1,PMD,directed = FALSE)
  
  Net1Link <- Net1$link_mass
  Net2Link <- Net2$link_mass
  #Net3Link <- Net3$link_mass

  colnames(Net1Link)[1:4]<-c('mz1','mz2','source','target')
  colnames(Net2Link)[1:4]<-c('mz1','mz2','source','target')
  #colnames(Net3Link)[1:4]<-c('mz1','mz2','source','target')
  Net1Link <- Net1Link[,c("source","target", "mz1", "mz2","PMD","PMD_cal","ElementReact","KVDReact","KVD2React")]
  Net2Link <- Net2Link[,c("source","target", "mz1", "mz2","PMD","PMD_cal","ElementReact","KVDReact","KVD2React")]
  #Net3Link <- Net3Link[,c("source","target", "mz1", "mz2","PMD","PMD_cal","ElementReact","KVDReact","KVD2React")]
  Graph1 <- graph_from_data_frame(Net1Link,directed = FALSE)
  Graph2 <- graph_from_data_frame(Net2Link,directed = FALSE)
  #Graph3 <- graph_from_data_frame(Net3Link,directed = FALSE)
  
  #GraphDiff <- Graph2 %u% Graph3 %m% Graph1
  GraphDiff <- Graph2 %m% Graph1
  #GraphDiff <- igraph::difference(Graph2,Graph1,byname=FALSE)
  link_mass <- data.frame(
    source=as.vector(ends(GraphDiff,E(GraphDiff))[,1]),
    target=as.vector(ends(GraphDiff,E(GraphDiff))[,2]),
    mz1 = edge.attributes(GraphDiff)$mz1,
    mz2 = edge.attributes(GraphDiff)$mz2,
    PMD = edge.attributes(GraphDiff)$PMD,
    PMD_cal = edge.attributes(GraphDiff)$PMD_cal,
    ElementReact = edge.attributes(GraphDiff)$ElementReact,
    KVDReact = edge.attributes(GraphDiff)$KVDReact,
    KVD2React = edge.attributes(GraphDiff)$KVD2React
  )
  #info_tmp <- edge.attributes(GraphDiff)
  #link_info <- as.data.frame(info_tmp[[1]])
  #colnames(link_info) <- colnames(Net2$link_mass)[3]
