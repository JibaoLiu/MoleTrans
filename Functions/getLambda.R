...
#fticr_data <- FtMsDf[[3]]['Formula']
#colnames(fticr_data)[1] <- 'MolForm'
# main run -------------------------------------------------------------
main_run <- function(fticr_data){
...
calc_r <- function(df, umax=1,`Vh[OC]`=1,`Vh[O2]`=1){
  df['rO2']<-0
  df['rOC']<-0
  df['rHCO3']<-0
  for (i in 1:nrow(df)) {
    df$rO2[i] <- abs(df$stoichMet_acceptor[i]*umax*exp(-abs(df$stoichMet_donor[i])/`Vh[OC]`)*exp(-abs(df$stoichMet_acceptor[i])/`Vh[O2]`))
    df$rOC[i] <- abs(df$stoichMet_donor[i]*umax*exp(-abs(df$stoichMet_donor[i])/`Vh[OC]`)*exp(-abs(df$stoichMet_acceptor[i])/`Vh[O2]`))
    df$rHCO3[i] <- abs(df$stoichMet_hco3[i]*umax*exp(-abs(df$stoichMet_donor[i])/`Vh[OC]`)*exp(-abs(df$stoichMet_acceptor[i])/`Vh[O2]`))
  }
  return(df)
}
#df <- calc_r(df)
#write.csv(df, file = "./output_result.csv", row.names=FALSE)

main_batch <- function(FtMsDf,VhO2=1){
  fticr_data <- FtMsDf[[1]]['Formula']
  colnames(fticr_data)[1] <- 'MolForm'
  df <- main_run(fticr_data)
  df <- calc_r(df,`Vh[O2]`=VhO2)
  df['sample'] <- names(FtMsDf)[1]
  if(length(FtMsDf)>1){
    for (i in 2:length(FtMsDf)) {
      fticr_data <- FtMsDf[[i]]['Formula']
      colnames(fticr_data)[1] <- 'MolForm'
      dftmp <- main_run(fticr_data)
      dftmp <- calc_r(dftmp)
      dftmp['sample'] <- names(FtMsDf)[i]
      df <- rbind(df,dftmp)
    }
  }
  return(df)
...
plot_histogram <- function(df,x="lambda",fill='sample',outputname='_',xlim=NULL){
  ggpar(gghistogram(df,x=x,y="..density..",bins = 300,add = "mean",rug = TRUE,add_density = TRUE,fill = fill,color=fill,alpha = 0.2,size = 6/80)+
          theme_prism(base_size = 8,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50,border = TRUE),xlim = xlim)+
          theme(legend.position = "top",legend.box.spacing=unit(0.1,"mm"),legend.key.height = unit(0.2,'cm'),legend.key.width = unit(0.3,'cm'))+
          scale_fill_manual(values = paletteer_dynamic("cartography::multi.pal", 20))+
          scale_color_manual(values = paletteer_dynamic("cartography::multi.pal", 20))
  ggsave(sprintf("./ggdensity_%s.pdf",outputname),dpi=300,units = 'cm',width = 8.5,height = 7)
}
#gghistogram(df,x="lambda",y="..density..",bins = 300,rug = TRUE,fill = "brown",alpha = 0.4,size = 6/80)+theme_prism(base_size = 8,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50,border = TRUE)
plot_histogram_rO2 <- function(df){
  ggpar(gghistogram(df,x="rO2",y="..density..",bins = 200,add = "mean",rug = TRUE,add_density = TRUE,color=NULL,fill = "sample",alpha = 0.2,size = 6/80)+theme_prism(base_size = 8,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50,border = TRUE),xlim = c(0,0.36))
  ggsave("./ggdensity_rO2.pdf",dpi=300,units = 'cm',width = 9.5,height = 7)
}
plot_histogram_rOC <- function(df){
  ggpar(gghistogram(df,x="rOC",y="..density..",bins = 200,add = "mean",rug = TRUE,add_density = TRUE,color=NULL,fill = "sample",alpha = 0.2,size = 6/80)+theme_prism(base_size = 8,base_fontface = "plain",base_line_size = 8/50,base_rect_size = 8/50,border = TRUE),xlim = c(0,0.36))
  ggsave("./ggdensity_rOC.pdf",dpi=300,units = 'cm',width = 9.5,height = 7)
}
